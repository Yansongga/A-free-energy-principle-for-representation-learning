...
def loss_function(recon_x, x, pred, y, mu, logvar, num_z):
    #xx = torch.cat([x.view(-1, 3, 32, 32)]* num_z)   #CIFAR10
    xx = torch.cat([x.view(-1, 1, 28, 28)]* num_z)  #MNIST
...
def train(epoch, num_z):
    ...
    for batch_idx, (data, labels) in enumerate(source_train_loader):
        ...
        KLD, MSE, c = loss_function(recon_batch, data, pred, labels, mu, logvar, num_z)       
...
def test(loader, num_z):
    args.model.eval()
    #test_loss = 0
    test_MSE, test_KLD, test_C, total, correct= 0., 0., 0., 0, 0
    with torch.no_grad():
        ...
        test_MSE /= (total /num_z)
        test_KLD /= (total /num_z)
        #test_C /= len(test_loader.dataset)
        test_C /= (total /num_z)

    return test_KLD, test_MSE, test_C, correct/total
    
def transfer(epoch, num_z):   
    #num_z = args.num_z
    args.model.train()
    for ( d1, d2 ) in zip ( source_train_loader,  target_train_loader):
        ...
        KLD2, MSE2, c2 = loss_function(recon2, x2, pred2, y2, mu2, logvar2, num_z)   
...
args.lumbda_max, args.lumbda_min, args.gamma= 2., 0.5, 2.
...
args.num_step, args.num_epoch = 25, 2


# In[ ]:


#####An equilibrium process for transfer learning: keeping the classicifation loss
